...
<div class="alert alert-info">
	当前位置<b class="tip"></b>媒体报道列表
</div>
<form id="searchForm" action="queryCMediaReportList">
	<table class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<tr>
				<td>标题</td>
				<td class="detail"><input id="title" name="title"
					type="text" value="${model.title }" />
				</td>
				<td>来源名称</td>
				<td class="detail"><input id="src" name="src"
					type="text" value="${model.src }" />
				</td>
				<td>是否删除</td>
			...
			<tr>
				<td colspan="6" align="right"><input class="btn btn-inverse"
					...
					onclick="goUrl('addCMediaReportIndex')" value="添加" />&nbsp;</td>
			</tr>
		</tbody>
	</table>
</form>
		...
		<c:forEach items="${pageBean}" var="bean" varStatus="st">
			<tr>
				...
				<td><input class="btn btn-inverse" type="button" value="修改"
					onclick="goUrl('updateCMediaReportIndex?id=${bean.id}');" />&nbsp; <input
					class="btn btn-inverse" type="button" value="查看"
					onclick="goUrl('queryCMediaReportIndex?id=${bean.id}');" />&nbsp;</td>
			</tr>
