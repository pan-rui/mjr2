							...
							<div class="new_cont">
								...
								<div class="new_box">
									<b class="notice_ico tran"></b> <span class="new_btit"><a
										href="notices-${bean.id}.html">${bean.noticeTitle}</a> </span> <span
										class="new_bdata"><fmt:formatDate
											value="${bean.createTime}" pattern="yyyy-MM-dd HH:mm:ss" />
									</span>
								</div>
							<!--标 end-->
							<div class="new_btxt"><my:delHTML len="120" content="${bean.noticeContent }"/> </div>
							</div>
							<!--box end-->
						</c:forEach>
					</div>
	...
	$(function(){
		...
		$(window).scroll(function() {
			...
			if($(window).scrollTop()>=468){
				...
				$(".p_sidebar").removeClass("fixedNav").css('top', '20px');
			} 
		  });
	});
//-->
